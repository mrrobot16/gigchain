...
export class Web3 {
  ...
  private signer: providers.JsonRpcProvider | Signer | undefined =
  ...
  public async deployOrgContract(
    ...
    console.log(
      "Deploying Org Contract... please confirm transaction in MetaMask."
    );
    const contract = new ContractFactory(
      OrganizationABI.abi,
      OrganizationABI.bytecode,
    ...
    callback != undefined
      ? callback(deployedOrgContractAddress as string)
      : null;
  }
  ...
  public async payOrgMembers(
    orgContractAddress: string,
    members: string[] | Member[],
    amounts: BigNumber[] | number[] | string[] | undefined
  ) {
    ...
    const tx = await contract.payMembers(members, amounts, {
      gasLimit: 1000000,
    });
    return tx;
  }
