...
import {
    ...
    ORGANIZATION_CONTRACT_FILE_PATH_V1,
    MEMBERS_V1_ACCOUNTS,
...
async function main() {
    ...
    const Organization = await ethers.getContractFactory(ORGANIZATION_CONTRACT_V1);
    const organization = await Organization.deploy(
        ORGANIZATION_NAME,
        MEMBERS_V1_ACCOUNTS.slice(0, MEMBERS.length - 1),
        { value: depositAmount }
    );
    ...
    const filePath = ORGANIZATION_CONTRACT_FILE_PATH_V1;
    const fileData = {
